...
def load_and_preprocess_data(datapath, filtered_features=None):
    """加载数据并进行预处理（支持特征筛选）"""
    df = pd.read_csv(datapath, encoding="gbk")  # delimiter=';',
    shared_globals.data=df
...
def  train(data_path,filtered_features):
    ...
    try:
        # 优先尝试原逻辑：使用SVMSMOTE
        print("Try using SVMSMOTE for oversampling...")
        sm = SVMSMOTE(random_state=shared_globals.random_seed)
        x_train, y_train = sm.fit_resample(x_train_raw, y_train_raw)
        print(f"SVMSMOTE oversampling successful! Shape of the training set after oversampling: {x_train.shape}")
        ...
        if "All support vectors are considered as noise" in str(e):
            ...
            print("Automatically switch to the basic SMOTE algorithm for oversampling...")
            # 切换为基础SMOTE，使用默认参数（兼容性更强）
            sm = SMOTE(random_state=shared_globals.random_seed, k_neighbors=5)
            x_train, y_train = sm.fit_resample(x_train_raw, y_train_raw)
            print(f"Basic SMOTE oversampling successful! Training set shape after oversampling: {x_train.shape}")
            print(f"Class distribution after oversampling:\n{pd.Series(y_train).value_counts()}")
        else:
