...
import os
from sklearn.metrics import classification_report  # 新增导入语句
...
def target_correlation_analysis(feature_dict, target_variable, column_names):  # Correlation between features and labels
    ...
    if len(unique_values) != 2:
        # 多分类：用Spearman相关
        for feature in column_names:
            corr, _ = spearmanr(target_variable, df[feature])
            feature_dict[feature]['target'] = corr
            target_variable_list.append(str(f"{feature} - target: {corr:.2f}"))
    else:
        # 二分类：用点二列相关
        for feature in column_names:
            corr, _ = stats.pointbiserialr(df[feature], target_variable)
            feature_dict[feature]['target'] = corr
            target_variable_list.append(str(f"{feature} - target: {corr:.2f}"))
...
def Fresh_breeze_parallel(x, column_names, min_list, max_list, count_list, init_val, x_list, model_weights, input_shape):
...
if __name__ == '__main__':
    ...
    # 2. 配置路径
    data_path = "dataset/DiabetesHealth.csv"
    model_save_path = "featureX_model/DiabetesHealth.keras"
    targetname = "Diabetes_binary"

    # 3. 加载原始数据（无论是否训练模型，都需要原始数据做相关性分析）
    df = pd.read_csv(data_path, encoding="gbk")
    column_names = df.columns[0:-1].tolist()  # 特征名称列表（后续步骤需用到）
    x_original, y_original = df.iloc[:, 0:-1], df.iloc[:, -1]  # 原始数据（未过采样）
    ...
    # 获取特征统计信息（用原始数据计算，确保统计量准确）
    min_list, max_list, count_list, range_list = value_range(column_names, df)
    # 生成扰动值范围
    ...
    # 准备基础输入（用原始数据的特征均值，作为扰动基准）
    base_data = df.iloc[:, 0:-1].mean().round().astype(int)
