...
def get_and_print_high_contrib_features(features, print_lock):
    ...
    with print_lock:
        ...
        for i, name in enumerate(high_contrib_names, 1):
            print(f"{i}. {name} (贡献值: {[f.con_i for f in features if f.name == name][0]:.4f})")  # 输出名称和贡献值
...
def build_causal_graph(features, corr_matrix, target_name, is_numeric,
    ...
    # 提前获取目标变量索引和最大贡献特征索引（核心变更1：提前计算，用于强制加入路径）
    target_idx = shared_globals.all_names.index(target_name[0])
    # 筛选非目标变量的特征索引
    feature_indices = [i for i in range(len(features)) if i != target_idx]
    max_con_idx = None
    max_con_feature = None
    if feature_indices:  # 确保存在非目标特征
        # 找到贡献值最大的非目标特征
        max_con_idx = max(feature_indices, key=lambda x: features[x].con_i)
        max_con_feature = features[max_con_idx]
        # 构建最大贡献特征→目标的路径（格式：[特征索引, 目标索引]）
        forced_path = [max_con_idx, target_idx]
    ...
    # 核心变更2：无论best_paths是否为空，强制加入最大贡献特征路径（去重）
    if max_con_idx is not None:
        # 路径去重：将路径转为元组（列表不可哈希），检查是否已存在
        best_paths_tuple = [tuple(path) for path in best_paths]
        forced_path_tuple = tuple(forced_path)
        if forced_path_tuple not in best_paths_tuple:
            best_paths.append(forced_path)
            with print_lock:
                print(f"\n强制加入最大贡献特征路径: {shared_globals.all_names[max_con_idx]} → {target_name[0]}")
        else:
            with print_lock:
                print(
                    f"\n最大贡献特征路径已存在: {shared_globals.all_names[max_con_idx]} → {target_name[0]}（无需重复添加）")
    else:
        with print_lock:
            print("\n无可用非目标特征，无法强制加入最大贡献路径")
...
def prune_paths(dag, features, all_names, target_name, current_alpha, is_numeric,
    ...
    with ThreadPoolExecutor(max_workers=min(os.cpu_count(), n_bootstrap)) as executor:
        ...
        boot_results = []
        # 使用executor.map替代pool.imap
        for result in executor.map(bootstrap_worker, args_list):
