...
def prune_paths(dag, features, all_names, target_name, current_alpha, is_numeric,
    ...
    # 关键修改：使用multiprocessing.Pool替代ThreadPoolExecutor
    with Pool(processes=min(os.cpu_count(), n_bootstrap)) as pool:
        with print_lock:
            progress_bar = tqdm(total=n_bootstrap, desc="Bootstrap抽样验证")

        # 使用imap实现异步迭代，提高效率
        boot_results = []
        for result in pool.imap(bootstrap_worker, args_list):
